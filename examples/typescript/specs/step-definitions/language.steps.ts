...
const feature = loadFeature(
  './examples/typescript/specs/features/language.feature',
);
...
defineFeature(feature, test => {
  ...
  describe('scenario-outlines', () => {
    test('Verkoop <Artikel> voor €<Bedrag>', ({given, when, then}) => {
      ...
      then(/^zou ik €(.*) ontvangen$/, expectedSalesPrice => {
  ...
  describe('using-dynamic-values', () => {
    ...
    test('Mijn salaris storten', ({given, when, then, pending}) => {
      given(/^mijn account balans is \€(.*)$/, balance => {
        myAccount.deposit(parseInt(balance, 10));
      });

      when(
        /^ik \€(.*) krijg betaald voor het schrijven van geweldige code$/,
        paycheck => {
          myAccount.deposit(parseInt(paycheck, 10));
        },
      );

      then(/^zou mijn account balans \€(.*) zijn$/, expectedBalance => {
  ...
  describe('using-gherkin-tables', () => {
    ...
    test('een artikel toevoegen aan mijn takenlijst', ({
      given,
      when,
      then,
    }) => {
      ...
      then('zou ik de volgende takenlijst zien:', (table: any[]) => {
        ...
        table.forEach((row: any, index) => {
          ...
          expect(todoList.items[index].priority).toBe(
