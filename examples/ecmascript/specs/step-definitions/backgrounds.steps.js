...
import {
  ArcadeMachine,
  COIN_TYPES,
} from '../../src/arcade-machine';

const feature = loadFeature(
  './specs/features/backgrounds.feature',
);
...
defineFeature(feature, (test) => {
  ...
  const givenMyMachineIsConfiguredToAcceptUsQuarters = (
    given,
  ) => {
    given(
      'my machine is configured to accept US Quarters',
      () => {
  ...
  test('Successfully inserting coins', ({
    given,
    when,
    then,
  }) => {
    ...
    then(
      /^I should have a balance of (.*) cents$/,
      (balance) => {
  ...
  test('Inserting a Canadian coin', ({ given, when, then }) => {
    ...
    when('I insert a Canadian Quarter', () => {
      coinStatus = arcadeMachine.insertCoin(
        COIN_TYPES.CanadianQuarter,
      );
    });
  ...
  test('Inserting a badly damaged coin', ({
    given,
    when,
    then,
  }) => {
